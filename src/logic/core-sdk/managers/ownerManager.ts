...
class OwnerManager {
  ...
  private validateAddressIsNotOwner(ownerAddress: string, owners: string[], errorMessage?: string): void {
  ...
  private validateAddressIsOwner(ownerAddress: string, owners: string[], errorMessage?: string): number {
    const ownerIndex = owners.findIndex((owner: string) => sameString(owner, ownerAddress))
    const isOwner = ownerIndex >= 0
    if (!isOwner) {
  ...
  async encodeSwapOwnerData(oldOwnerAddress: string, newOwnerAddress: string): Promise<string> {
    ...
    return this.safeContract.encode('swapOwner', [prevOwnerAddress, oldOwnerAddress, newOwnerAddress])
  }
