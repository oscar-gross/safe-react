...
class Safe {
   ...
   */
  static async create({ ethAdapter, safeAddress, isL1SafeMasterCopy, contractNetworks }: SafeConfig): Promise<Safe> {
   ...
   * @throws "MultiSend contract is not deployed on the current network"
   */
  private async init({ ethAdapter, safeAddress, isL1SafeMasterCopy, contractNetworks }: SafeConfig): Promise<void> {
   ...
   * @throws "MultiSend contract is not deployed on the current network"
   */
  async connect({ ethAdapter, safeAddress, isL1SafeMasterCopy, contractNetworks }: ConnectSafeConfig): Promise<Safe> {
  ...
  async signTransactionHash(hash: string): Promise<SafeSignature> {
    ...
    const addressIsOwner = owners.find((owner: string) => signerAddress && sameString(owner, signerAddress))
    if (!addressIsOwner) {
   ...
   */
  async approveTransactionHash(hash: string, options?: TransactionOptions): Promise<TransactionResult> {
    const owners = await this.getOwners()
    const signerAddress = await this.ethAdapter.getSignerAddress()
    const addressIsOwner = owners.find((owner: string) => signerAddress && sameString(owner, signerAddress))
    if (!addressIsOwner) {
   ...
   */
  async getEnableModuleTx(moduleAddress: string, options?: SafeTransactionOptionalProps): Promise<SafeTransaction> {
   ...
   */
  async getDisableModuleTx(moduleAddress: string, options?: SafeTransactionOptionalProps): Promise<SafeTransaction> {
    const safeTransaction = await this.createTransaction({
      to: this.getAddress(),
      value: '0',
   ...
   * @throws "Threshold cannot exceed owner count"
   */
  async getChangeThresholdTx(threshold: number, options?: SafeTransactionOptionalProps): Promise<SafeTransaction> {
    const safeTransaction = await this.createTransaction({
      to: this.getAddress(),
      value: '0',
   ...
   * @throws "Cannot specify gas and gasLimit together in transaction options"
   */
  async executeTransaction(safeTransaction: SafeTransaction, options?: TransactionOptions): Promise<TransactionResult> {
    const txHash = await this.getTransactionHash(safeTransaction)
